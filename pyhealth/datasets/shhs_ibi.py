...
from pyhealth.tasks.shhs_ibi_sleep_staging import SleepStagingSHHSIBI
...
class SHHSIBIDataset(BaseDataset):
    ...
    Examples:
        >>> from pyhealth.datasets import SHHSIBIDataset
        >>> dataset = SHHSIBIDataset(root="/path/to/shhs")
        >>> dataset.stats()
    """
    ...
    def __init__(
        ...
        if config_path is None:
            config_path = str(Path(__file__).parent / "configs" / "shhs_ibi.yaml")
        ...
        super().__init__(
            root=root,
            tables=["shhs_ibi_sleep"],
            dataset_name=dataset_name or "shhs_ibi_sleep",
    ...
    @property
    def default_task(self) -> SleepStagingSHHSIBI:
        return SleepStagingSHHSIBI()
